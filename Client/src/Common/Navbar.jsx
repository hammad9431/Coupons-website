...
function Navbar() {
  const [activeIndex, setActiveIndex] = useState(null); // tracks which li is active
...
<div className="h-12 px-4 w-full bg-blue-600 text-center text-white hidden lg:flex items-center justify-end gap-2 text-2xl cursor-pointer">
      <a href="https://www.facebook.com/couponsghana?_rdc=1&_rdr#" target="_blank" rel="noopener noreferrer">
      <CiFacebook className="hover:text-[#1877F2] transition-colors cursor-pointer" />
       </a>
       <a href="https://x.com/CouponsGhana">
       <CiTwitter className="hover:text-[#1DA1F2] transition-colors" />
       </a>
       <a href="https://www.instagram.com/couponsghana/">
  ...
  <div className="top-full -mt-11 w-max md:hidden hidden xl:inline lg:inline bg-[#4776ff] text-white z-40 left-1/2 -translate-x-1/2 absolute">
    <div className="max-w-300 mx-auto py-4 px-6">
      <div className="grid grid-cols-3 text-md text-left">
        ...
        <ul>
          {["Restaurants and Food Delivery", "Entertainment", "Sports and Fitness", "Clothing and Fashion", "Home Improvement"].map((item, idx) => (
            <li
              key={idx}
              onClick={() => setActiveIndex(idx)}
              className={`w-full flex items-center py-2 px-4 cursor-pointer hover:bg-[#1746d3] ${
                activeIndex === idx ? "bg-[#1746d3]" : ""
              }`}
            >
              {item}
            </li>
        ...
        <ul className="border-l border-white/40">
          {["Services", "Theater and Cinema", "Gadgets and Electronics", "Health and Wellness", "General Merchandize"].map(
            (item, idx) => {
              const globalIdx = idx + 5; // unique index
              return (
                <li
                  key={globalIdx}
                  onClick={() => setActiveIndex(globalIdx)}
                  className={`w-full flex items-center py-2 px-4 cursor-pointer hover:bg-[#1746d3] ${
                    activeIndex === globalIdx ? "bg-[#1746d3]" : ""
                  }`}
                >
                  {item}
                </li>
        ...
        <ul className="border-l border-white/40">
          {["Tourism and Hospitality", "Education", "Transport"].map((item, idx) => {
            const globalIdx = idx + 10; // unique index
            return (
              <li
                key={globalIdx}
                onClick={() => setActiveIndex(globalIdx)}
